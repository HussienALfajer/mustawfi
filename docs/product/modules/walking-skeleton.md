...
- **Slice 4 (2026-09-25).**
  ...
  - **ADR-0017 amended (needs the user's acceptance):** the policy is `tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid`. After a transaction-local `set_config`, a pooled connection returns `''`, not `NULL`, so the ADR's original expression raised a uuid cast error without a context instead of returning zero rows (verified on 18.6; the catalog test flags the old form).
