...
- **Slice 12 (2026-09-25).**
  - **Where it lives:** ADR-0026 names `packages/testing/sync-sim`, but packages may not import modules (ADR-0015 rule 4). The generic harness is `packages/testing/src/sync-sim` (`@mustawfi/testing/sync-sim`): `simRandom` (seeded streams, `fork` per actor), `createSimulatedNetwork` (a `fetch` per link), `runSimulation` (weighted steps with a trace), `converge`, and `convergenceProblems` (plain snapshots). The scenario, the composition of the real modules and server, is `apps/server/sync-sim/sync-sim.test.ts`. It has its own tsconfig (DOM and JSX, because module client entries include screens); the server's `typecheck` runs both, so server code stays without DOM types.
  - **The scenario:** the real Fastify server listens on HTTP over Testcontainers PostgreSQL 18. Three devices each run `node:sqlite` in memory, the real local migrations, `registerThisDevice`, `addToCart`/`completeCashSale`, and `createSyncEngine` through `createApiSyncTransport` (the real `apiRequest` path) over the device's simulated link. Steps: sell, sync one device, a burst (every device syncs while late requests land), toggle a device offline, deliver late requests, the owner adds a product, receive stock. Device clocks are skewed by up to ±10 minutes. Then `converge` heals the network, delivers every held request, and syncs until nothing is pending.
  - **Faults per request (10% each):** request lost; answer lost after the server handled it; duplicated (both copies sent at once, the client gets either answer); delayed (the client times out, and the request lands later, shuffled among other late requests, after newer ones). Offline spells come on top. A link's faults come from its own stream, so they depend only on the seed and what that device sent. Server-generated ids and prefixes stay random, so a replay repeats the faults and choices, not the ids, and not the server-side order of the concurrent requests in a `burst` step.
  - **Checks:** every device invoice is on the server once, with its number, device, and total, and ends `accepted` or `duplicate` (never pending or rejected); no server invoice that no device made; numbers are gapless per device; each invoice with a total has one balanced entry for that total, no entry posts a non-invoice, and the ledger balances; stock on hand = received − sold; every device holds every product; no round ends `failed`; **no 5xx answer at all**, including answers nobody receives. The test also requires that every fault kind happened and that some invoice ended `duplicate`, so the idempotency path really ran.
  - **Budget:** seeds 1, 2, 3 × 300 steps; each seed must converge within 30 s (`SYNC_SIM_BUDGET_MS`). About 4 s each locally, around 50–60 invoices per seed. Replay: `SYNC_SIM_SEEDS=<n>`; longer runs: `SYNC_SIM_STEPS`.
  - **Mutation checks (run by hand, reverted):** the client marking operations accepted before sending → lost invoices (2 of 3 seeds); the server skipping the stored-`opId` lookup → rejected duplicates (3/3); pull applying one change per page → devices missing products (3/3); no device lock *and* no mapping from unique violations to rejections → `core.server.internal` and 5xx (3/3). Removing only the device lock is **not** caught: the rejection path re-reads `received_ops` and answers `duplicate`, so the lock stays covered by `sync.test.ts`.
  - Production changes: `apiRequest` takes an optional `fetch`; `core.sync/client` gains `createApiSyncTransport({ fetch })` (`apiSyncTransport` is its default). Not covered yet (`core-sync` unit): partitions of the server itself, restart or restore of a device's database (the only source of `deviceSeq` gaps), long outages across session expiry, `fc.commands` model-based runs, and the 72-hour test.
