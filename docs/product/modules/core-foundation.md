...
- **Slice 9 (2026-09-26).**
  ...
  - The guard gains `deviceEvenRevoked` beside `device`: push and the wipe report accept a revoked credential; everything else answers 401 `access.device.revoked` (pull, `/devices/current`, password and PIN sign-in on the device), so the device learns why. **Deviation (interpretation of rule 23 and ADR-0030's "every other API refuses"):** the wipe report is the one call besides push that a revoked credential may make, since the rule asks the device to report its wipe.
  ...
  - **Deviation from `screen-patterns.md`:** the notice pattern is used here for the first time without a preview approved before its code; its screenshots (light and dark) are in the slice report for the user's review. Details panels still have no «last changed by … on …» (slice 17).
...
- 2026-09-26 — Slice 9 done (device revoke with flags and wipe, devices screen, removal notice).
