...
- **Permissions declared after a tenant exists** — a module enabled later, or a permission a later slice adds, reaches no existing role: template roles got their grants when the tenant was created. Decide in slice 6 (or `core-config`) whether template roles pick up new template grants, and how that respects an owner's removals.
