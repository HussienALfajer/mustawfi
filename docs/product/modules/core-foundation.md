...
- **Slice 10 (2026-09-26).**
  ...
  - Screens: the sign-in form's second step (heading «التحقق بخطوتين», one field for either kind of code, back); the recovery form at `/recover` (store code, login, reset code, new password twice, optional new PIN), linked from sign-in and returning to it with a notice; «My account» at `/account` (`AccountScreen`), reached by a «حسابي» link in the top bar; the users panel shows each user's 2FA state and gives owners «إلغاء التحقق بخطوتين…» with a confirmation and a reason. Arabic labels for the four `access.twoFactor.*` audit actions. The e2e journey adds a second owner through the API so the other journeys' owner keeps a password-only sign-in; the e2e license allows 20 users.
  - **Deviations from `screen-patterns.md`:** «My account» has no sticky Save footer and no leave guard — each section (PIN, password, 2FA) proves and saves on its own, since each needs its own current secret, and nothing typed is worth keeping. The QR code keeps the light theme's colours in the dark theme (`data-theme="light"` on its box), because authenticator apps read dark modules on light. The top bar has a «حسابي» link beside the user's name instead of a user menu: the menu comes with switching user (slice 15), when it has three entries.
