...
- **Slice 8 (2026-09-26).**
  ...
  - **`TRUST_PROXY`** (server configuration): addresses or CIDR ranges of the reverse proxies whose `X-Forwarded-For` gives `request.ip`; none by default. A deployment behind a proxy must set it, or every client shares the proxy's address and the per-address limit throttles them together (`ops`).
