...
**Policies:** every tenant-owned table has `ENABLE` and `FORCE ROW LEVEL SECURITY` and a policy `USING (tenant_id = current_setting('app.tenant_id', true)::uuid)` with the same `WITH CHECK`. A missing setting yields `NULL`, which matches no row and blocks every insert — the failure mode is "nothing", never "everything". In V1 the policies filter by tenant only; branch filtering joins them when multi-branch arrives.
