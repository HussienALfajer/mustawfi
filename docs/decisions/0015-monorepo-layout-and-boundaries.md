...
- 2026-09-25 (walking-skeleton slice 16, rule 5 check): rule 5 is checked by `tools/boundaries/src/schemas.ts` inside `pnpm check:boundaries`, not by dependency-cruiser — import edges cannot tell a table export from another export, and SQL is text. `no-table-export` walks each module entry's exports with the TypeScript compiler (re-exports, namespaces, aliases included) and fails on a Drizzle table, view, enum, sequence, or schema — client SQLite tables too. `own-schema-only` fails when a module's migrations, `sql` templates, SQL strings, or `pgSchema(...)` name another module's schema (`core.ledger` → `core_ledger`), except a `REFERENCES` toward a module in `dependsOn` (ADR-0016). Test files are not checked.
