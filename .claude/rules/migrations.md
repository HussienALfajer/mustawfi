...
- Each module keeps its migrations in `<module>/migrations/` (drizzle-kit layout; `drizzle-kit` lives in `tools/drizzle`). `apps/server/src/db/migrate.ts` applies them in dependency order and refuses an applied migration whose checksum changed.
...
- Every tenant-owned table has `tenant_id`, `branch_id`, `created_at`, `created_by`; `ENABLE` and `FORCE ROW LEVEL SECURITY`; and a policy `USING (tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid)` with the same `WITH CHECK` (the `nullif` matters: a pooled connection returns `''`, not `NULL`, after an earlier context). The catalog test fails on a table missing `tenant_id`, `branch_id`, forced RLS, or the policy.
