...
- One ledger per tenant. Departments are a dimension on journal lines, never separate books. Other modules post only through `core.ledger`'s public posting interface, `postJournalEntry` (`@mustawfi/core-ledger/server`), called in the document's transaction, never by writing its tables.
